...
def validate_due_date(doc):
	# validate the due date according to priority
	priority = frappe.db.get_value("Issue", doc.reference_name, "priority")
	days = frappe.db.get_value("Priority", priority, "days")
	if not priority or not days:
		pass
	else:
		from frappe.utils import getdate, add_days
		doc.date = add_days(getdate(), int(days))
		doc.priority = priority

	""" RELIANCE HELPDESK ESCALATION WISE DUE DATE """
