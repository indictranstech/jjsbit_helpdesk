...
def send_to_approval_notification(doc, method):
	if doc.workflow_state == "Sent to Approval" and doc.resolution_details and doc.approval_mail==0:
		...
		if email :
			msg = """ Dear '%s, <br><br> Ticket - '%s' is Raised By '%s' related to Branch - '%s' and Category - '%s'. Please verify this ticket and do appropriate action against it. <br><br> Thank You, <br> JJSB Helpdesk Ticketing System"""%(email, doc.name, doc.raised_email, doc.branch, doc.department)
			doc.approval_mail = 1
			frappe.sendmail(recipients=email, subject="Ticket Approval Notification", content=msg)

def approved_notification(doc, method):
	if doc.workflow_state == "Approved" and doc.resolution_details and doc.approved_mail==0:
		email = frappe.db.get_single_value("CC User Settings", "email_id")
		if email :
			msg = """ Dear '%s, <br><br> Ticket - '%s' is Approved By Authorized User - '%s' related to Branch - '%s' and Category - '%s'. <br><br> Thank You, <br> JJSB Helpdesk Ticketing System"""%(email, doc.name, frappe.session.user, doc.branch, doc.department)
			doc.approved_mail = 1
			frappe.sendmail(recipients=email, subject="Approved Ticket Notification", content=msg)

def rejection_notification(doc, method):
	if doc.workflow_state == "Rejected" and doc.resolution_details and doc.rejected_mail==0:
		email = frappe.db.get_single_value("CC User Settings", "email_id")
		if email :
			msg = """ Dear '%s, <br><br> Ticket - '%s' is Rejected By Authorized User - '%s' related to Branch - '%s' and Category - '%s'. <br><br> Thank You, <br> JJSB Helpdesk Ticketing System"""%(email, doc.name, frappe.session.user, doc.branch, doc.department)
			doc.rejected_mail = 1
			frappe.sendmail(recipients=email, subject="Ticket Rejection Notification", content=msg)
